...
def norm_sev(s: str) -> str:
    s = (s or "").upper()
    if s in BUCKETS: return s
    # common mappings
    if s in ("ERROR","SEVERE"): return "HIGH"
    if s in ("WARN","WARNING"): return "MEDIUM"
    if s in ("INFO","INFORMATIONAL","INFORMATION"): return "LOW"
    # ZAP risk codes
    if s in ("3","2","1","0"):
        return {"3":"HIGH","2":"MEDIUM","1":"LOW","0":"LOW"}[s]
        return "LOW"

        seen  = set()def load_json(p: pathlib.Path):
        details = []
    ...
    def spotbugs(data, fn):
        ...
        if isinstance(bugs, dict): bugs=[bugs]
        for b in bugs:
    ...
    for fp in files:
        ...
        for name, handler in routers:
            if name in fn:
                if isinstance(data, (dict,list)): handler(data, fn)
                break
    ...
    out_dir.joinpath("overall_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")

    md = []
    md.append(f"### Overall Severity (deduped across tools)\n**Threshold:** `{min_severity}` → **Decision:** `{decision}`\n")
    ...
    for n in summary["inputs"]:
        md.append(f"- {n}")
    out_dir.joinpath("overall_summary.md").write_text("\n".join(md), encoding="utf-8")
