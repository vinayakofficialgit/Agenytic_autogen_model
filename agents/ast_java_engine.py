...
class ASTJavaEngine:
    ...
    # =========================================================
    # ✅ FIX: Extended classifier to handle Semgrep rule patterns
    #
    # Before: only matched "sql injection" literally
    # After:  also matches "formatted-sql-string", "sql-string",
    #         "sqli", "jdbc", etc.
    # =========================================================
    def _classify(self, title: str) -> Optional[str]:
        t = title.lower()

        # SQL Injection — expanded patterns
        sql_keywords = [
            "sql injection", "sql-injection", "sqli",
            "formatted-sql", "sql-string", "sql_string",
            "jdbc", "createstatement", "executequery",
            "hibernate-sqli", "jpa-sqli", "tainted-sql",
        ]
        if any(kw in t for kw in sql_keywords):
            return "SQL_INJECTION"
        # Also catch "sql" + "inject" anywhere in the string
        if "sql" in t and "inject" in t:
            return "SQL_INJECTION"

        # Command Injection — expanded patterns
        cmd_keywords = [
            "command injection", "command-injection",
            "cmd-injection", "os-command",
            "runtime.exec", "processbuilder",
        ]
        if any(kw in t for kw in cmd_keywords):
            return "COMMAND_INJECTION"
        if ("command" in t or "cmd" in t) and ("inject" in t or "exec" in t):
            return "COMMAND_INJECTION"
