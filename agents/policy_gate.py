# # policy_gate.py
# """
# PolicyGate evaluates normalized findings and decides whether to fail the pipeline and/or open a remediation PR.

# Inputs (flexible):
#   - Typically receives `findings` from main.py (CollectorAgent or similar)
#   - OR can read a pre-merged file agent_output/merged_findings.json (when run as __main__)

# Outputs:
#   - agent_output/decision.json  -> {"open_pr": bool, "status": "ok|fail", "reason": str, "stats": {...}}
#   - agent_output/policy_summary.md (human-readable)
#   - agent_output/policy_explained.md (optional, if LLM_EXPLAIN=1 and llm_bridge.py available)

# Exit code (only when run as a script):
#   - 0 on pass, 1 on fail (as per policy)

# Tuning (via environment variables):
#   - MIN_SEVERITY => "low|medium|high|critical" (default: "high")
#   - MAX_FINDINGS_TOTAL => integer limit (optional)
#   - MAX_PER_CATEGORY_code/infra/image/policy/secrets/webapp => per-category caps (optional)
#   - ALLOW_TOOLS => comma-separated tools to ignore in gating (still counted in stats)
#   - DENY_TOOLS => comma-separated tools that always count in gating (deny wins over allow)
#   - SEVERITY_WEIGHTS => JSON mapping e.g. {"low":1,"medium":2,"high":3,"critical":5}
#   - LLM_EXPLAIN => "1" to also generate a short explanation via llm_bridge/Ollama (optional)
# """
...
# # --- Robust LLM import: works if llm_bridge is in agents/ or at repo root ---
# try:
...
#     except Exception:
#         assistant_factory = None  # type: ignore
...
#     """
#     Lightweight helper to ask the LLM (Ollama via assistant_factory).
...
#     # future: exemptions, path filters, age-of-findings, etc.


# def load_policy_config() -> PolicyConfig:
#     # Base config from env
#     min_sev = os.getenv("MIN_SEVERITY", "high").strip().lower()
#     max_total = _int_env("MAX_FINDINGS_TOTAL", None)

#     # Per-category caps: env names like MAX_PER_CATEGORY_code, MAX_PER_CATEGORY_infra, ...
...
#     allow_tools = _csv_env("ALLOW_TOOLS")  # example: "semgrep,tfsec"
#     deny_tools = _csv_env("DENY_TOOLS")    # example: "gitleaks"
...
#     # Optional weighted score (for future ranking/reporting)
...
# def gate(stats: Dict[str, Any], findings: List[Dict[str, Any]], cfg: PolicyConfig) -> Tuple[bool, str]:
#     """
#     Returns (should_fail, reason)
#     Gating rules (in order):
#       1) Any finding with severity >= MIN_SEVERITY (after deny/allow filters) -> fail
#       2) Total count > MAX_FINDINGS_TOTAL -> fail
#       3) Per-category count > MAX_PER_CATEGORY_<cat> -> fail
#     ALLOW_TOOLS: entries here are IGNORED for gating (but counted in stats).
#     DENY_TOOLS: entries here always count in gating (deny wins).
#     """
#     min_rank = _sev_rank(cfg.min_severity)
#     allow = set((cfg.allow_tools or []))
#     deny = set((cfg.deny_tools or []))

#     # 1) Severity gate (tool allow/deny applied)
#     for f in findings:
#         tool = (f.get("tool") or f.get("source") or "").strip().lower()
#         sev_rank = _sev_rank(f.get("severity"))
#         # apply allow/deny filter: deny wins
#         if tool in allow and tool not in deny:
#             continue
#         if sev_rank >= min_rank:
#             loc = f.get("location", "")
#             return True, f"Found {f.get('severity')} (>= {cfg.min_severity}) from {tool}{(' at ' + loc) if loc else ''}"
...
# # --------------------------
# # OO wrapper used by main.py
# # --------------------------
# class PolicyGate:
#     """
#     Thin OO wrapper so main.py can call:
#         decision = PolicyGate(cfg, output_dir).decide(findings)

#     It writes:
#         - decision.json
#         - policy_summary.md
#         - (optional) policy_explained.md (if LLM_EXPLAIN=1 and llm_bridge is available)
#     into output_dir.
#     """
...
#     def _maybe_llm_explain(self, stats: Dict[str, Any], reason: str) -> Optional[str]:
#         """
#         Optionally generate a short explanation via llm_bridge (Ollama).
#         Controlled by env var LLM_EXPLAIN=1.
#         """
...
#             path.write_text(_llm_banner() + text, encoding="utf-8")
#             return str(path)
#         return None

#     def _write_outputs(self, should_fail: bool, reason: str, stats: Dict[str, Any]) -> Dict[str, Any]:
...
#             "stats": stats,
#         }
#         (self.out_dir / "decision.json").write_text(json.dumps(decision, indent=2), encoding="utf-8")
...
#             md.append(f"  - {k}: {stats['by_severity'].get(k, 0)}")
...
#         explained_path = self._maybe_llm_explain(stats, reason)
#         if explained_path:
#             decision["policy_explained_md"] = explained_path

#         return decision

#     def decide(self, findings: List[Dict[str, Any]]) -> Dict[str, Any]:
#         # Load env-driven policy (MIN_SEVERITY, caps, etc.)
#         policy_cfg = load_policy_config()
#         stats = summarize(findings, policy_cfg)
#         should_fail, reason = gate(stats, findings, policy_cfg)
#         return self._write_outputs(should_fail, reason, stats)
...
#     # Try to load merged findings first
...
#             findings = data
#     else:
#         # Fallback: try to collect from ./reports using your own collector if present
#         try:
#             # Prefer agents.collector (package layout), fallback to collector at root
#             try:
#                 from agents.collector import CollectorAgent  # type: ignore
#             except Exception:
#                 from collector import CollectorAgent  # type: ignore
#             agent = CollectorAgent({"dedup_keys": ["tool", "id", "location"]}, Path("reports"), out_dir)
#             findings = agent.load_all()
#         except Exception as e:
#             print(f"[policy_gate] Warning: could not collect findings from reports: {e}")
#             findings = []

#     decision = PolicyGate({}, out_dir).decide(findings)
