...
public class InsecureController {
  ...
  public List<Map<String, Object>> search(@RequestParam String name) {
    String sql = "SELECT * FROM USERS WHERE NAME = ?"; // fixed
  ...
  public String ping(@RequestParam String host) throws IOException, InterruptedException {
    // Validate host input to allow only specific hosts or patterns
    if (!isValidHost(host)) {
  ...
  private boolean isValidHost(String host) {
    // Implement validation logic (e.g., allowlist specific hosts)
    return host.matches("^[a-zA-Z0-9.-]+$"); // Example regex for basic validation
  }
