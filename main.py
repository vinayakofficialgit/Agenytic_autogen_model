...
def main():
    ...
    # =====================================================
    # 6️⃣ AUTOFIX (no PR here — workflow YAML handles it)
    # =====================================================
    if mode in ("fix", "all") and decision["decision"] == "FAIL":
        ...
        # Apply all fixes (deterministic + IaC + LLM Java)
        try:
            with suppress():
        ...
        # =====================================================
        # ✅ CHANGED: Removed GitPRAgent PR creation.
        # The workflow YAML handles branch + commit + PR
        # in a single step to ensure ONE PR with ALL fixes.
        # =====================================================
        if len(patches) > 0:
            print(f"[autofix] {len(patches)} file(s) patched — workflow will create PR")
        else:
            print("[autofix] no files changed — no PR needed")
