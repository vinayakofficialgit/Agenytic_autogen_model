...
# 🔥 Import severity ranking from policy_gate (single source of truth)
try:
...
def main():
    ...
    # 🔥 SEVERITY FILTERING (Display + AI aligned)
    min_sev = (
        os.getenv("MIN_SEVERITY")
        or cfg.get("policy", {}).get("min_severity_to_fail")
        or "critical"
    ).strip().lower()
    ...
    filtered_grouped = {}
    for tool, items in findings_grouped.items():
        filtered = []
        for item in items:
            if _sev_rank(item.get("severity")) >= min_rank:
                filtered.append(item)
        if filtered:
            filtered_grouped[tool] = filtered

    findings_grouped = filtered_grouped
    ...
    if decision.get("status") == "fail":
        try:
            with suppress_verbose_output():
                Fixer(cfg, output_dir).apply(findings_grouped)
        except Exception:
            pass
    ...
    print("\n==============================")
    print("Pipeline Status:", decision.get("status", "ok").upper())
    print("==============================")
