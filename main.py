...
# # main.py
# """
# DevSecOps Agentic AI Pipeline - Enhanced with Clean Two-Phase Output

# Output Structure:
#   PHASE 1: Scan Results & Decision (clean summary)
#   PHASE 2: AI-Powered Suggestions & Best Practices (detailed remediation)

# Key Features:
# - Clean, professional output
# - Suppressed verbose LLM retry messages
# - Two-phase output: Results first, then suggestions
...
#         pass  # Silent fail for .env loading


# # --- Suppress verbose output context manager ---
# @contextlib.contextmanager
# def suppress_verbose_output():
#     """Suppress stdout during LLM calls to keep output clean."""
#     if os.getenv("LLM_VERBOSE", "0") == "1":
#         yield  # Don't suppress if verbose mode
#     else:
#         # Capture and discard stdout
...
#     """
#     print("\n" + "━" * 70)
#     print("   📊  PHASE 1: SECURITY SCAN RESULTS")
#     print("━" * 70)
...
#             if v.get("message"):
#                 print(f"      └─ {v['message'][:60]}{'...' if len(v.get('message', '')) > 60 else ''}")
...
#         print(f"   📝 All findings within acceptable thresholds")
#     print("━" * 70 + "\n")


# def print_phase2_suggestions(llm_report: dict, decision: dict):
#     """
#     PHASE 2: Print AI-powered suggestions and best practices.
#     """
#     print("\n" + "━" * 70)
#     print("   🤖  PHASE 2: AI-POWERED REMEDIATION SUGGESTIONS")
#     print("━" * 70)
    
#     if not llm_report:
#         print("\n   ℹ️  No LLM analysis available.")
#         print("━" * 70 + "\n")
#         return
...
#     if fallback_count > 0:
#         print(f"\n   ⚠️  Note: Using built-in security knowledge (LLM unavailable)")
#         print(f"       To enable AI suggestions, ensure Ollama is running properly.\n")
...
#             sev_icon = "🔴" if sev == "CRITICAL" else "🟠" if sev == "HIGH" else "🟡" if sev == "MEDIUM" else "🟢"
#             print(f"\n   {sev_icon} [{sev}] {item.get('file')}:{item.get('line')} ({item.get('rule_id')})")
#             print(f"   {'─'*60}")
            
#             suggestion = item.get("suggestion", "")
#             # Clean up the suggestion text
#             suggestion = suggestion.replace("[Fallback - LLM unavailable]\n\n", "")
#             suggestion = suggestion.replace("[Fallback - LLM unavailable]", "")
...
#     # Trivy-FS suggestions
#     trivy_items = llm_report.get("trivy_fs", [])
#     if trivy_items:
...
#             sev = (item.get("severity") or "").upper()
#             sev_icon = "🔴" if sev == "CRITICAL" else "🟠" if sev == "HIGH" else "🟡" if sev == "MEDIUM" else "🟢"
#             print(f"\n   {sev_icon} [{sev}] {item.get('file')} ({item.get('id')})")
#             print(f"   {'─'*60}")
...
#       • Never use 0.0.0.0/0 CIDR blocks in production
...
#     files = [
#         ("decision.json", "Pipeline decision and full report"),
#         ("llm_report.json", "AI analysis results"),
#         ("llm_recommendations.md", "Detailed remediation suggestions"),
#         ("policy_summary.md", "Policy evaluation summary"),
#         ("pr_comment.md", "Ready-to-use PR comment"),
#         ("metrics.json", "Scan metrics and statistics"),
#     ]
...
#         print("   ⚠️  Please review and fix the security issues above")
#     else:
...
# def main():
#     """Main entry point for the DevSecOps pipeline."""
#     # Load .env
...
#     parser.add_argument("--verbose", "-v", action="store_true", help="Enable verbose LLM output")
#     parser.add_argument("--skip-llm", action="store_true", help="Skip LLM analysis")
#     parser.add_argument("--no-best-practices", action="store_true", help="Skip best practices section")
...
#     # 3) AutoGen LLM layer (quiet mode - suppress retry messages)
#     llm_report = None
...
#     # ═══════════════════════════════════════════════════════════════════
#     # CLEAN OUTPUT - TWO PHASES
#     # ═══════════════════════════════════════════════════════════════════
    
#     # PHASE 1: Show scan results and decision
#     print_phase1_results(findings_grouped, decision)
    
#     # PHASE 2: Show AI suggestions and remediation
#     print_phase2_suggestions(llm_report, decision)
    
#     # PHASE 3: Best practices (optional)
#     if not args.no_best_practices:
