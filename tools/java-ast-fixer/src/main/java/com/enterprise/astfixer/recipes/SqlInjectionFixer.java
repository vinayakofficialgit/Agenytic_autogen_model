...
public class SqlInjectionFixer implements AstRecipe {
    ...
    public boolean apply(CompilationUnit cu, AstContext ctx, AstResult result) {
        ...
        cu.findAll(MethodCallExpr.class).forEach(mc -> {
            ...
            // Convert to parameterized SQL (append a single placeholder)
            ...
            Optional<VariableDeclarator> stmtDecl = mc.findAncestor(VariableDeclarator.class);
            if (stmtDecl.isEmpty()) return;
            VariableDeclarator vd = stmtDecl.get();

            // Replace type: Statement -> PreparedStatement
            vd.setType(new ClassOrInterfaceType(null, "PreparedStatement"));

            // Replace initializer
            // Use the same statement's connection instead of hardcoding 'conn'
            // e.g., stmt.getConnection().prepareStatement("SELECT ... ?")
            String scopeExpr = mc.getScope().map(Object::toString).orElse("stmt");
            vd.setInitializer(
                StaticJavaParser.parseExpression(
                    scopeExpr + ".getConnection().prepareStatement(" + newSql + ")"
                )
            );

            // Insert binding before executeQuery
            Optional<BlockStmt> parentBlock = mc.findAncestor(BlockStmt.class);
            if (parentBlock.isPresent()) {
                BlockStmt block = parentBlock.get();
                Statement bindStmt = StaticJavaParser.parseStatement(
                    vd.getNameAsString() + ".setString(1, " + paramVar + ");"
                );
